        ...
        if file_name[-4:] == "json":
            ...
            with open(f_path, 'r') as f:
                ...
                for i in range(max_distance):
                    ...
                    denominator[i] += (0.001+df[idx][i][1]+df[idx][i][0])
...
def draw_conditional_probability(x, y):
    ...
    plt.title("Kyoto, Scenario: 4 to 1")
    plt.show()
...
def generate_points_for_fitting(x, y, y_min=1e-5, y_max=0.02):
...
def draw_fitting(x, y, x_draw, y_draw, idx, slope):
    ...
    plt.title("Kyoto-" + key_to_type1_dict[idx])
    plt.text(15, 0.002, '$r$=' + str(round(slope, 2)), fontsize=12, color='black', ha='center', va='bottom')
    plt.savefig('log-log-figure/20250105/' + idx + '_'  + key_to_type1_dict[idx] + '_' + str(round(slope, 2)) + '.svg',bbox_inches = 'tight')
    plt.show()
